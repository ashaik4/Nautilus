...
def load_all_geolocation_info(ip_version=4, tags='default'):
	...
	with open('stats/location_data/maxmind_location_output_v{}_{}'.format(ip_version, tags), 'rb') as fp:
	...
	with open('stats/location_data/ripe_location_output_v{}_{}'.format(ip_version, tags), 'rb') as fp:
	...
	if ip_version == 4:
		print ('Next, we will load the geolocation results from CAIDA')

		with open('stats/location_data/caida_location_output_{}'.format(tags), 'rb') as fp:
	...
	if latitude and longitude:
		...
		if prev_examined_location:
			...
			current_contents = ip_location_with_penalty_and_total_count.get(ip,
								...
								'penalty_count': [0] * 11, 'total_count': [0] * 11})
