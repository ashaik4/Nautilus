...
def save_maxmind_output(maxmind_location, ip_version=4, tags='default'):
	...
	with open('stats/location_data/maxmind_location_output_v{}_{}'.format(ip_version, tags), 'wb') as fp:
...
def generate_locations_for_list_of_ips (ips_list, ip_version=4, tags='default'):
	...
	mmdb_file = 'stats/location_data/GeoLite2-City.mmdb'
	...
	if Path(mmdb_file).exists():
		...
		print (f'File not found. The mmdb file should be downloaded from Maxmind city database and saved as stats/location_output/GeoLite2-City.mmdb')
		return (None, None)
...
def load_maxmind_output(ip_version=4, tags='default', ips_list=[]):
	...
	file_location = 'stats/location_data/maxmind_location_ouput_v{}_{}'.format(ip_version, tags)
