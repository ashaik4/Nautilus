...
# Use a generator to process lines lazily
...
def save_caida_geolocation_output(caida_location, tags='default'):
    ...
    with open('stats/location_data/caida_location_output_{}'.format(tags), 'wb') as fp:
        pickle.dump(caida_location, fp)


def get_node_to_geolocation_map():

    ip_to_geo_map_path = 'stats/location_data/caida_ip_to_geo_map'
    ...
    if Path(ip_to_geo_map_path).exists():
        ...
        midar_iff_nodes_file_path = 'stats/location_data/midar-iff.nodes'
        caida_nodes_to_geo_file_path = 'stats/location_data/midar-iff.nodes.geo'
        ...
        if Path(midar_iff_nodes_file_path).exists() and Path(caida_nodes_to_geo_file_path).exists():
            ...
            with open(midar_iff_nodes_file_path) as fp:
                ...
                for line in fp: 
                    ...
                    parts = line.split()
                    if len(parts) > 2:
                        nodes_to_ip_dict[parts[1].strip(':')] = parts[2:]
            ...
            #     # print(line)
            #     updated_content.append(line.split())
            # nodes_to_ip_dict = {line[1].strip(':'): line[2:] for line in updated_content}


            # nodes_to_ip_dict = dict(process_lines(contents[start_index:]))
            ...
            with open(caida_nodes_to_geo_file_path) as fp:
                ...
                for line in fp: 
                    ...
                    if len(line_parts) > 9: 
                        key = line_parts[0].split()[-1].strip(':')
                        value = (
                            line_parts[1],
                            line_parts[2],
                            line_parts[4],
            ...
            # updated_content = [line.split('\t') for line in contents[start_index:]]
            # nodes_to_geo_dict = {line_parts[0].split()[-1].strip(':'): (line_parts[1], line_parts[2], line_parts[4], line_parts[5], line_parts[6], line_parts[9].strip()) for line_parts in updated_content}
...
def load_caida_geolocation_output (tags='default', ips_list = []):
    ...
    file_location = 'stats/location_data/caida_location_output_{}'.format(tags)
...
if __name__ == '__main__':
    ...
    # with open(f'stats/mapping_outputs/all_ips_v{ip_version}', 'rb') as fp:
